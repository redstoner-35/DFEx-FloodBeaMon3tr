...
void LEDControlHandler(void)
	...
	if(IsThermalStepdown())
		...
		if(BoostModeInfoTIM>16)BoostModeInfoTIM=0;  //定时器大于16，清零到初始值
			...
			RLED=0;
			GLED=0;
			return;            //在特定时刻，让LED短暂熄灭并退出
			}		
		else BoostModeInfoTIM++; //计时器继续累加
		}
	//进行狂暴模式工作指示
	else if(QueryIsSystemInBoostMode())
		{
	...
	switch(LEDMode)
		...
		case LED_AmberBlinkFifth:
			...
			if(timer>((LEDMode==LED_RedBlinkThird||LEDMode==LED_GreenBlinkThird)?6:10))LEDMode=LED_OFF; //时间到，关闭识别
				...
				if(LEDMode==LED_AmberBlinkFifth){GLED=1;RLED=1;}
				else if(LEDMode==LED_GreenBlinkThird){GLED=1;RLED=0;}
				else {GLED=0;RLED=1;}
				}		
			else 
				{
