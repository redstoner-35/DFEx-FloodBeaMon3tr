...
static int SetFanPWMProcess(void)
	...
	if(!IsEnable2SMode)buf*=DualLit1SFanLimitRatio;
	...
	if(buf>FanPWMMaxRatio)buf=(float)FanPWMMaxRatio;            //应用降额系数限制风扇最高速度
	if(buf<VMinMaxCfg.SysMinSpeed)buf=VMinMaxCfg.SysMinSpeed; 	//对速度参数进行百分比限幅				
