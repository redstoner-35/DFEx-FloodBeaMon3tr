  ...
  ModeBeforeFSMSwitch=CurrentMode->ModeIdx;		 //存下进入之前的挡位
	...
	switch(ModeBeforeFSMSwitch)
		...
		case Mode_OFF:
		  ...
		  //系统处于过热锁定状态或者温度感应故障，禁止开机
			if(!QueryIfSysThermalIsOK())	
				{
				if(ClickCount||getSideKeyLongPressEvent())LEDMode=LED_AmberBlinkFifth;  //其余任意按键次数以及长按，全部闪五次
				else switch(getSideKeyNClickAndHoldEvent())	
					{
					case 0:break; //按键没按
					case 2:TriggerVshowDisplay();break; //双击+长按查询电量
					case 3:
						if(Data.IsNTCOK)TriggerTShowDisplay();
				    else LEDMode=LED_AmberBlinkFifth;
					  break; //三击+长按查询温度(仅温度传感器正常时)					
					//其余情况
					default:LEDMode=LED_AmberBlinkFifth;
					}
				//直接打断禁止后面的执行
				break;
				}
