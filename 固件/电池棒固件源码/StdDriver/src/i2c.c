...
static bit I2C_WaitACK(void)
	...
	while(!(I2CMSR & I2C_I2CMSR_I2CMIF_Msk))
      ...
      _nop_(); //复位寄存器之后需要等待				
			...
			I2C_ConfigCLK(0x0B); //Fsclk=Fsys/(2*10*(8+1))=266.66KHz (这芯片I2C太快会导致卡死)		
			return 1;	
			}
		//倒计时
		j--;
...
//通信结束
	...
	do
		{
		//I2C通信已结束，返回0
		if(I2CMSR & I2C_I2CMSR_IDLE_Msk)return 0;		
		//当前状态没有置起Flag，等待
    delay_ms(1);
		}			
	while(--Len);	
  ...
  _nop_(); //复位寄存器之后需要等待				
	I2C_EnableMasterMode(); //启用主控发送模式
	I2C_ConfigCLK(0x0B); //Fsclk=Fsys/(2*10*(8+1))=200KHz (这芯片I2C太快会导致卡死)					
	return 1;	
	}
