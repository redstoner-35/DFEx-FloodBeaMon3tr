...
**	History:
				2026年7月8日 11:46   
														 修改电池电压检测的上拉电阻至820K，预留更大幅度的
														 检测余量以避免用户错误接入高压电池导致ADC烧毁。
														 （该更改为硬件更改因此配合软件端的操作）

				2026年4月28日 14:40
														 1.修改电池电压检测参数改为使用4串电池。	
														 2.新增设置电池串数的宏定义方便更改固件的电池检测
															 配置参数。
														 3.修改输出电压检测的阻值至2颗470K串联，和硬件保持
															 一致（针对高压输出可以降低电阻压力）
...
//ADC外部采集的参数配置
#define BattCellCount 4  					//驱动固件目标的三元锂电池节数
#define VoutUpperResK (470*2)
#define VoutLowerResK 56 			//输出检测分压的上下拉电阻(KΩ)
#define VBattUpperResK 820
...
bit DoVOUTSampleOnly;  //标志位，仅做输出电压转换
...
code unsigned char ADCConvAvgCMD[ADCConvertQueueDepth]=
	...
	2,    //FB注入电压反馈，追求最快转换速度只采样2次
	6,    //电池电压需要较为平滑的滤波，多采几次
	2     //输出电压反馈，追求最快转换速度只采样2次
	};
...
static void ADC_WriteOutputBuf(int ADCResult,char Ch)
  ...
  switch(Ch)
		...
		case VBATInputAIN:
		  ...
		  Data.BatteryVoltage=Data.RawBattVolt/(float)BattCellCount; //将4节电池的总电压转换为单节电池的电压
...
static void ADCEngineHandler(void)
  ...
  do
        ...
        //正式准备上传			
				...
				for(i=0;i<ADCConvertQueueDepth;i++)
					 {
					 if(DoVOUTSampleOnly)ADCConvertQueue[i]=(i==4)?VOUTFBAIN:0xFF;  //开启仅转换输出电压模式
			     else ADCConvertQueue[i]=ADCConvChCMD[i]; 									//把转换命令储存里面的数据复制过去
					 }
		  ...
		  //向ADC转换线程提交任务
	        ...
	        if(DoVOUTSampleOnly&&Ch!=VOUTFBAIN)        //当前开启仅电压转换模式，但是上一个队列仍未结束，里面包含其他转换，立即强制结束当前队列跳转到新的队列内容
						{
						ADCState=ADC_SubmitQueue;
						break;
						}
