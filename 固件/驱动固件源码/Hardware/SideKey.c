...
unsigned char ShortPressCount;
unsigned char ShortPressEvent;
...
//获取侧按键点按次数的获取函数
unsigned char getSideKeyShortPressCount(void)
  {
...
//获取按键单击+长按次数的操作
unsigned char getSideKeyNClickAndHoldEvent(void)
	{
	//检测到多击+长按事件，返回对应多击次数
	if(IsLargerThanOneU8((unsigned char)Keyevent.HoldStat))return (unsigned char)Keyevent.HoldStat-1;
	//没有事件发生，返回0
	return 0;
	}
/*************************  End Of File  ***********************/
